/**
 * Safe formula parser to avoid `eval()`.
 * Supports basic arithmetic, (), SUM, AVERAGE, MIN, MAX.
 */
export class FormulaParser {
  static evaluate(formula: string): number {
    try {
      ...
      // Very restricted Function constructor acts as a safe math evaluator
      // Only digits, operators, and dots are left.
      if (!/^[\d\.\+\-\*\/\(\)\s]+$/.test(f)) {
